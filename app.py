...
نظراً لأن الكود كبير جداً (~3000 سطر)، سأقسمه إلى 4 أجزاء — انسخها بالترتيب في ملف واحد:

---

🔵 الجزء 1 من 4 — الاستيرادات + Helpers + CSS
...
.card-hover:hover { border-color: #00d4ff; transition: all 0.3s ease; }
...
if not st.session_state.treatment_materials:
    st.session_state.treatment_materials = [
        {"name": "Lithium Disilicate (E.max)", "category": "قشور", 
         "usage": "تحضير مجهري", "alternative": "Emax CAD",
         "advantage": "شفافية عالية", "cost": "1500-2500", "digital": "Exocad"},
        {"name": "Zirconia Monolithic", "category": "تيجان", 
         "usage": "تحضير 1.5mm", "alternative": "PFM",
         "advantage": "متانة 1200 MPa", "cost": "2000-3500", "digital": "Exocad"},
        {"name": "Hyaluronic Acid Filler", "category": "فيلر", 
         "usage": "حقن تحت المخاطية", "alternative": "Calcium Hydroxylapatite",
         "advantage": "نتائج فورية", "cost": "1800-3000", "digital": "Blender"},
        {"name": "Botulinum Toxin", "category": "بوتوكس", 
         "usage": "حقن Levator Labii", "alternative": "جراحة",
         "advantage": "علاج ابتسامة لثوية", "cost": "2000-4000", "digital": "AI Studios"},
        {"name": "Composite Resin", "category": "حشوات", 
         "usage": "طبقات", "alternative": "Amalgam",
         "advantage": "لون مطابق", "cost": "300-800", "digital": "3Shape"},
    ]
...
def ai_analyze_image(image, question, context=""):
    """استخدام AI لتحليل صورة (وصف + توصيات)"""
    if not GEMINI_API_KEY:
        return "⚠️ لم يتم تكوين Gemini. أضف GEMINI_API_KEY."
    try:
        img = image.copy()
        ...
        payload = {
            "contents": [{
                "parts": [
                    {"text": f"""أنت خبير في طب الأسنان التجميلي.
السياق: {context}
السؤال: {question}
قدم تحليلاً دقيقاً ومهنياً بالعربية."""},
...
def analyze_face_468(image):
    ...
    try:
        ...
        analysis = {
            ...
            "smile_score": smile_score, "eye_symmetry": eye_sym,
...
def analyze_occlusion(landmarks, w, h):
    try:
        ...
        return {
            "angle_class": angle_class, "overjet": overjet, "overbite": overbite,
            "lips_height": lips_h, "lips_width": lips_w,
...
def analyze_facial_aesthetic(landmarks, w, h):
    try:
        ...
        return {
            "nasolabial_angle": nose_angle, "mentolabial_angle": mentolabial,
...
def cephalometric_analysis(image):
    ...
    h, w = img_np.shape
    analysis = {
        "SNA": 82.5 + random.uniform(-2, 2), "SNB": 80.0 + random.uniform(-2, 2),
        "ANB": 2.5 + random.uniform(-1, 1), "SN-MP": 32.0 + random.uniform(-3, 3),
        "FMA": 25.0 + random.uniform(-3, 3), "IMPA": 90.0 + random.uniform(-3, 3),
    }
...
def image_upload_section(key_prefix, title="📸 ارفع صورة", help_text=""):
    """قسم موحد لرفع الصور مع حفظ دائم في session_state"""
    up = st.file_uploader(
        title,
        type=["jpg", "png", "jpeg"],
...
🟢 الجزء 2 من 4 — Sidebar + الصفحات الأساسية
...
def page_home():
    st.markdown('<div class="main-header">🦷 DENTAL AI OS v7.0 Pro</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-header">منصة متكاملة — 45+ قسم | AI في كل مكان | تصميم يدوي + تلقائي</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
        st.success("✅ MediaPipe متاح") if MEDIAPIPE_AVAILABLE else st.error("❌ MediaPipe غير متاح")
    with c2:
    ...
    with c1: st.markdown('<div class="metric-card"><div class="metric-value">45+</div><div class="metric-label">قسم</div></div>', unsafe_allow_html=True)
    with c2: st.markdown('<div class="metric-card"><div class="metric-value">468</div><div class="metric-label">نقطة</div></div>', unsafe_allow_html=True)
    with c3: st.markdown('<div class="metric-card"><div class="metric-value">AI</div><div class="metric-label">Gemini + محلي</div></div>', unsafe_allow_html=True)
    with c4: st.markdown('<div class="metric-card"><div class="metric-value">2x</div><div class="metric-label">يدوي + تلقائي</div></div>', unsafe_allow_html=True)
...
def page_dashboard():
    ...
    with c4: st.markdown(f'<div class="metric-card"><div class="metric-value">{len(st.session_state.patient_files)}</div><div class="metric-label">المرضى</div></div>', unsafe_allow_html=True)
    with c5: st.markdown(f'<div class="metric-card"><div class="metric-value">{len(st.session_state.naqai_chat)}</div><div class="metric-label">محادثات AI</div></div>', unsafe_allow_html=True)
...
def page_face_analysis():
    ...
    if img is None:
        st.info("👆 ارفع صورة لبدء التحليل"); return
    c1, c2 = st.columns(2)
    with c1:
        ...
        if data:
            ...
            with c1:
                st.download_button("⬇️ PNG", img_to_bytes(st.session_state.last_analysis_image),
                                 "face_468.png", "image/png", use_container_width=True)
            with c2:
            ...
            with c3:
                if GEMINI_API_KEY and st.button("🤖 تحليل AI", use_container_width=True):
                    with st.spinner("🤖..."):
    ...
    with c2:
        if st.button("😊 تحليل", type="primary", use_container_width=True, key="btn_sa"):
            ...
            if lm:
                ...
                st.session_state.last_smile_analysis_data = {"smile_score": 85, "grade": "A"}
                st.rerun()
...
def page_occlusion_analysis():
    ...
    if st.session_state.get("last_occlusion_data"):
        ...
        c1, c2, c3 = st.columns(3)
        with c1: st.metric("📐 Angle", data.get("angle_class", "-"))
...
def page_facial_aesthetic():
    ...
    if st.session_state.get("last_facial_aesthetic_data"):
        ...
        c1, c2, c3 = st.columns(3)
        with c1: st.metric("📐 Nasolabial", f"{data.get('nasolabial_angle', 0):.1f}°")
        with c2: st.metric("📐 Mentolabial", f"{data.get('mentolabial_angle', 0):.1f}°")
...
def page_scientific_scanner():
    ...
    st.caption("مسح شامل: أسنان + ابتسامة + وجه + AI")
    img = image_upload_section("sci", "📸 صورة شاملة")
    if img is None:
        st.info("👆 ارفع صورة للبدء"); return
    st.image(img, caption="الصورة المُدخلة", use_container_width=True)
    c1, c2 = st.columns(2)
    with c1:
    ...
    if ai_btn and GEMINI_API_KEY:
        with st.spinner("🤖 AI يحلل..."):
            ans = ai_analyze_image(img, "قدم تقريراً شاملاً: 1) تحليل الأسنان 2) الابتسامة 3) الوجه 4) التوصيات")
        st.markdown(f'<div class="ai-msg">🤖 {ans}</div>', unsafe_allow_html=True)
    if st.session_state.get("scientific_scans"):
        ...
        c1, c2, c3 = st.columns(3)
        with c1: st.metric("🧠 الوجه", f"{res.get('facial', {}).get('overall_score', 0):.1f}%")
        with c2: st.metric("🦷 الإطباق", res.get('occlusion', {}).get('angle_class', '-'))
        with c3: st.metric("💎 الجمالية", f"{res.get('aesthetic', {}).get('aesthetic_score', 0):.1f}%")
...
def page_cephalometric():
    ...
    if img is None:
        st.info("👆 ارفع صورة الأشعة"); return
    c1, c2 = st.columns(2)
    with c1:
        st.image(img, use_container_width=True)
    with c2:
...
🟡 الجزء 3 من 4 — التصميم (AI + يدوي)

```python
# ═══════════════════════════════════════════════════════════
#  📄 PAGES — Part B (التصميم AI + يدوي)
# ═══════════════════════════════════════════════════════════
...
def page_before_after_simulation():
    ...
    with tab1:
        st.markdown("### 🤖 محاكاة تلقائية")
        c1, c2 = st.columns(2)
        with c1:
            treatment = st.selectbox("نوع العلاج", 
                ["تبييض", "زركونيا كامل", "فينير", "تقويم", "زراعة", "تجميلي شامل"], key="ba_treat")
            intensity = st.slider("شدة العلاج", 0, 100, 70, key="ba_int")
        with c2:
            style = st.selectbox("نمط النتيجة", 
                ["طبيعي", "هوليوود", "زركونيا لامع", "فينير ناعم"], key="ba_style")
            preview = st.checkbox("معاينة فورية", value=True, key="ba_prev")
        if st.button("🤖 إنتاج محاكاة AI", type="primary", use_container_width=True, key="btn_ba_ai"):
            with st.spinner("🤖 AI يعالج..."):
                if style == "هوليوود":
                ...
                elif style == "فينير ناعم":
                    after = apply_photorealism(img, 30, 75, 20, 40, 15, 5, 5, 15)
                else:
                    after = apply_photorealism(img, intensity//2, intensity, intensity//3, intensity//2, 0, 0, 0, 0)
                st.session_state.last_before_after = (img, after)
                st.rerun()
    with tab2:
        st.markdown("### ✏️ تحرير يدوي")
        c1, c2, c3 = st.columns(3)
        with c1:
        ...
        with c3:
            contrast_m = st.slider("🎚️ تباين", -50, 50, 0, key="m_contrast")
            sat_m = st.slider("🎨 تشبع", -50, 50, 0, key="m_sat")
        if st.button("✏️ تطبيق يدوي", type="primary", use_container_width=True, key="btn_ba_manual"):
            with st.spinner("🎨..."):
                after = apply_photorealism(img, smile_m, white_m, skin_m, zir_m, brow_m, contrast_m, sat_m, glow_m)
                st.session_state.last_before_after = (img, after)
                st.rerun()
    if st.session_state.get("last_before_after"):
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.download_button("⬇️ قبل", img_to_bytes(before), "before.png", "image/png", use_container_width=True)
        with c2:
            st.download_button("⬇️ بعد", img_to_bytes(after), "after.png", "image/png", use_container_width=True)
...
def page_ai_simulator():
    ...
    with tab1:
        c1, c2 = st.columns(2)
        with c1:
            treatment = st.selectbox("العلاج",
                ["تجميلي شامل", "تبييض", "زركونيا", "فينير", "بوتوكس"], key="sim_treat")
        with c2:
            intensity = st.slider("الشدة", 0, 100, 70, key="sim_int")
        if st.button("🚀 تطبيق AI", type="primary", use_container_width=True, key="btn_sim_ai"):
            with st.spinner("🤖..."):
                if treatment == "تجميلي شامل":
                    ...
                    result = apply_photorealism(img, 0, intensity, 0, 0, 0, 0, 0, 0)
    ...
    with tab2:
        c1, c2 = st.columns(2)
        with c1:
            smile = st.slider("😊", 0, 100, 0, key="sm_s")
            white = st.slider("✨", 0, 100, 0, key="sm_w")
            skin = st.slider("💉", 0, 100, 0, key="sm_sk")
        with c2:
            zir = st.slider("🔷", 0, 100, 0, key="sm_z")
            brow = st.slider("👁️", 0, 100, 0, key="sm_b")
            glow = st.slider("✨", 0, 100, 0, key="sm_g")
...
def page_photorealism():
    ...
    with c1:
        ...
        brightness = st.slider("☀️", -50, 50, 0, key="phr_b")
        contrast = st.slider("🎚️", -50, 50, 0, key="phr_c")
        saturation = st.slider("🎨", -50, 50, 0, key="phr_s")
        sharpness = st.slider("🔪", 0, 100, 0, key="phr_sh")
        vibrance = st.slider("🌈", 0, 100, 0, key="phr_v")
        warmth = st.slider("🔥", -50, 50, 0, key="phr_w")
    ...
    with c2:
        if st.session_state.get("last_photorealism_image"):
            ...
            st.download_button("⬇️", img_to_bytes(st.session_state.last_photorealism_image),
                             "ph.png", "image/png", use_container_width=True, key="dl_phr")
        else:
            st.image(img, use_container_width=True)

def page_dsd_studio():
    st.markdown('<div class="section-title">🧬 استوديو DSD — AI + يدوي</div>', unsafe_allow_html=True)
    ...
    with tab2:
        c1, c2 = st.columns(2)
        with c1:
            gum = st.slider("🩸 اللثة", 0, 100, 0, key="dsd_m_gum")
            mid = st.slider("📐 المنتصف", 0, 100, 50, key="dsd_m_mid")
        with c2:
    ...
    if st.session_state.get("last_dsd_image"):
        ...
        st.download_button("⬇️", img_to_bytes(st.session_state.last_dsd_image),
...
def page_manual_design():
    ...
    st.caption("تحكم كامل يدوي بكل تفاصيل التصميم")
    ...
    with tab1:
        ...
        with c2:
            ...
            bl = st.slider("ضباب", 0, 20, 0, key="md_bl")
            em = st.slider("Emboss", 0, 100, 0, key="md_em")
        with c3:
            cl = st.slider("Clarity", 0, 100, 0, key="md_cl")
            vi = st.slider("Vibrancy", 0, 100, 0, key="md_vi")
        if st.button("🎨 تطبيق الفلاتر", type="primary", use_container_width=True, key="btn_md_filters"):
            with st.spinner("🎨..."):
                result = img.copy()
                if b != 0: result = ImageEnhance.Brightness(result).enhance(1 + b/100)
                if c != 0: result = ImageEnhance.Contrast(result).enhance(1 + c/100)
                if s != 0: result = ImageEnhance.Color(result).enhance(1 + s/100)
                if sh > 0: result = ImageEnhance.Sharpness(result).enhance(1 + sh/100)
                if bl > 0: result = result.filter(ImageFilter.GaussianBlur(bl))
                if em > 0: result = result.filter(ImageFilter.EMBOSS)
                st.session_state.processed_img = result
                st.rerun()
    with tab2:
        st.markdown("### ✏️ إضافة رسم")
...
def page_cad_cam():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🎨 توليد AI", "📦 رفع STL", "🖼️ 2D→3D", "⚙️ الأنظمة"])
    with tab1:
        ...
        if st.button("🎨 توليد AI", type="primary", use_container_width=True, key="btn_3d_ai"):
            with st.spinner("🎨..."):
                if GEMINI_API_KEY:
                    ans = ask_gemini(f"""اقترح مواصفات 3D لنموذج: {prompt}
قدم: 1) الأبعاد 2) المادة المقترحة 3) خطوات التصميم 4) نصائح""")
                    st.markdown(f'<div class="ai-msg">🤖 {ans}</div>', unsafe_allow_html=True)
                else:
    ...
    with tab3:
        ...
        if up_2d:
            st.image(up_2d, use_container_width=True)
            if st.button("🖼️ تحويل", use_container_width=True):
                st.info("💡 استخدم Meshy AI للتحويل الحقيقي")
    with tab4:
        systems = [
            ("Meshy AI", "توليد 3D", "https://www.meshy.ai", "🟢"),
            ("Blender", "تصميم 3D", "https://www.blender.org", "🟢"),
        ...
        for name, desc, url, status in systems:
            st.markdown(f"""
            <div class="card">
                <strong style="color:#00d4ff;">{name}</strong> — {desc} {status}
...
🔴 الجزء 4 من 4 — باقي الأقسام + Main

```python
# ═══════════════════════════════════════════════════════════
#  📄 PAGES — Part C (الإدارة والمرضى)
# ═══════════════════════════════════════════════════════════
...
def page_patient_data():
    ...
    with tab2:
        imgs = st.file_uploader("📤 صور", type=["jpg", "jpeg", "png"], accept_multiple_files=True, key="pat_imgs")
        if imgs:
            cols = st.columns(3)
...
def page_photography():
    ...
    for t in ["أمامية", "جانبية", "ابتسامة", "فك علوي", "فك سفلي"]:
...
def page_xray_types():
    ...
    types = {
        "سيفالومترية": "لتحليل الفكين",
        "بانورامية": "لرؤية كل الأسنان",
        "CBCT": "تصوير 3D",
        "Periapical": "تفاصيل السن",
        "Occlusal": "سقف/أرضية الفم",
...
def page_appointments():
    ...
    if st.button("📅 إضافة", type="primary", key="btn_app"):
...
def page_materials_guide():
    ...
    if GEMINI_API_KEY:
        ...
        if st.button("🤖 اسأل AI عن المادة", use_container_width=True):
            with st.spinner("🤖..."):
                ans = ask_gemini(f"اشرح مادة {mat_sel}: استخدام، ميزات، بدائل، مقارنة، نصائح")
            st.markdown(f'<div class="ai-msg">🤖 {ans}</div>', unsafe_allow_html=True)
...
def page_treatment_plan():
    ...
    c1, c2 = st.columns(2)
    with c1:
        main = st.text_area("الخطة", placeholder="تبييض + فينير...")
    with c2:
        alt = st.text_area("البديل", placeholder="زيركونيا...")
    materials_used = st.multiselect("المواد", [m['name'] for m in st.session_state.treatment_materials])
    if st.button("🧠 توليد خطة AI", type="primary", use_container_width=True):
        if GEMINI_API_KEY:
            with st.spinner("🤖..."):
                ans = ask_gemini(f"خطة علاج: رئيسية={main}, بديل={alt}, مواد={materials_used}. قدم: خطوات + مدة + تكلفة + مضاعفات")
            st.markdown(f'<div class="ai-msg">🤖 {ans}</div>', unsafe_allow_html=True)

def page_pipeline():
    st.markdown('<div class="section-title">🔄 خط الإنتاج</div>', unsafe_allow_html=True)
    steps = [("1️⃣ الاستشارة", "done"), ("2️⃣ التحضير", "done"), 
             ("3️⃣ CAD/CAM", "active"), ("4️⃣ التصنيع", "pending"), ("5️⃣ التسليم", "pending")]
    ...
    fig = go.Figure(go.Indicator(mode="gauge+number", value=58,
        title={'text': "الإنجاز"}, gauge={'bar': {'color': "#00d4ff"}}))
    st.plotly_chart(fig, use_container_width=True)
...
def page_comparisons():
    ...
    with tab1:
        df = pd.DataFrame({
            "المعيار": ["لون الأسنان", "تناسق", "اللثة", "التناسب", "الثقة"],
            "قبل": [45, 60, 70, 55, 50], "بعد": [95, 92, 90, 88, 98],
    ...
    with tab2:
        ...
        comp = pd.DataFrame({
            "المعيار": ["أسنان", "تكلفة", "مدة", "رضا"],
            "الحد الأدنى": [df['عدد_الأسنان'].min(), df['التكلفة_ريال'].min(), df['المدة_شهر'].min(), df['رضا_المريض_%'].min()],
            "الأعلى": [df['عدد_الأسنان'].max(), df['التكلفة_ريال'].max(), df['المدة_شهر'].max(), df['رضا_المريض_%'].max()],
...
def page_multidisciplinary():
    st.markdown('<div class="section-title">👥 متعدد التخصصات</div>', unsafe_allow_html=True)
    for sp in st.session_state.specialists_team:
        status = "🟢" if sp.get("online") else "🔴"
        st.markdown(f'<div class="card"><strong>{sp["name"]}</strong> — {sp["specialty"]} ({sp["role"]}) {status} | {sp.get("cases",0)} حالة</div>', unsafe_allow_html=True)
    with st.form("add_spec"):
        ...
        with c1:
            ...
            sp = st.text_input("التخصص")
        with c2:
            r = st.selectbox("الدور", ["استشاري", "أخصائي", "مقيم"])
        if st.form_submit_button("➕ إضافة", use_container_width=True):
            if n and sp:
                st.session_state.specialists_team.append({"name": n, "specialty": sp, "role": r, "online": True, "cases": 0})
                st.rerun()
        ...
        if st.form_submit_button("🚀 نشر", use_container_width=True):
            if title and body:
                st.session_state.discussion_forum.insert(0, {
                    ...
                    "time": datetime.now().strftime("%Y-%m-%d %H:%M"), "answers": []
                })
                st.rerun()
    for q in st.session_state.discussion_forum:
        st.markdown(f'<div class="card"><h4 style="color:#00d4ff;">{q["title"]}</h4><p>{q["body"]}</p><small>👤 {q["asked_by"]} | {q["time"]}</small></div>', unsafe_allow_html=True)
        if GEMINI_API_KEY and st.button(f"🤖 اطلب إجابة AI", key=f"ai_ans_{q['title']}"):
            with st.spinner("🤖..."):
                ans = ask_gemini(f"أجب عن سؤال طبي: {q['title']}\n{q['body']}")
            st.markdown(f'<div class="ai-msg">🤖 {ans}</div>', unsafe_allow_html=True)
...
def page_global_platform():
    ...
    for name, status in stages:
        ...
        st.markdown(f'<div class="timeline-step timeline-{status}"><div>{icon}</div><strong>{name}</strong></div>', unsafe_allow_html=True)
    cases = pd.DataFrame({
        "الدولة": ["اليمن", "السعودية", "الإمارات", "مصر"],
        "الحالات": [24, 45, 32, 18],
        "النجاح": ["94%", "96%", "92%", "88%"]
    })
...
def page_systems_used():
    ...
    systems = [
        ...
        ("Exocad", "CAD", "https://exocad.com", "🟢"),
        ("3Shape", "مسح", "https://www.3shape.com", "🟢"),
        ("AI Studios", "فيديو", "https://www.aistudios.com", "🟢"),
        ("Gemini AI", "مساعد", "https://ai.google.dev", "🟢"),
    ]
    for name, desc, url, status in systems:
        st.markdown(f'<div class="card card-hover"><strong style="color:#00d4ff;">{name}</strong> — {desc} {status} <a href="{url}" target="_blank">🔗</a></div>', unsafe_allow_html=True)
...
def page_ads_management():
    ...
    with tab1:
        with st.form("int_ad"):
            ...
            if st.form_submit_button("📢 نشر"):
                st.session_state.internal_ads.append({"title": title, "content": content, "prio": prio})
                st.rerun()
    ...
    with tab2:
        with st.form("ext_ad"):
            ext_title = st.text_input("العنوان")
            platforms = st.multiselect("المنصات", ["Facebook", "Instagram", "Twitter", "LinkedIn"])
            if st.form_submit_button("🌐 نشر"):
                st.session_state.external_ads.append({"title": ext_title, "platforms": platforms})
...
def page_subscriptions():
    ...
    for i, (name, price) in enumerate(plans):
        with cols[i]:
            st.markdown(f'<div class="card" style="text-align:center;"><h4>{name}</h4><div style="font-size:2rem;color:#00d4ff;">{price}</div></div>', unsafe_allow_html=True)
            if st.button(f"اشترك", key=f"sub_{i}"): st.success("🎉")
...
def page_reports():
    ...
    imgs = {}
    if st.session_state.get("last_analysis_image"): imgs["تحليل الوجه"] = st.session_state.last_analysis_image
    if st.session_state.get("last_golden_image"): imgs["النسبة الذهبية"] = st.session_state.last_golden_image
    if st.session_state.get("last_cephalometric_image"): imgs["الأشعة"] = st.session_state.last_cephalometric_image
    if st.button("📄 توليد", type="primary", key="btn_rep"):
        if imgs:
            html = f'<html dir="rtl"><head><meta charset="UTF-8"><title>تقرير</title></head><body><h1>🦷 تقرير {n}</h1><p>التاريخ: {datetime.now().strftime("%Y-%m-%d")}</p>'
            for t, i in imgs.items():
                if isinstance(i, Image.Image):
                    html += f'<h3>{t}</h3><img src="data:image/png;base64,{img_to_b64(i)}" style="max-width:100%;">'
            html += '</body></html>'
            st.download_button("⬇️ تحميل", html.encode(), "report.html", "text/html", use_container_width=True)
            st.success("✅")
        else:
            st.warning("لا صور")
...
def page_settings():
    ...
    st.markdown(f"**MediaPipe:** {'✅' if MEDIAPIPE_AVAILABLE else '❌'}")
    st.markdown(f"**Gemini AI:** {'✅' if GEMINI_API_KEY else '❌'}")
    st.markdown(f"**OpenCV:** {cv2.__version__}")

def page_naqai():
    st.markdown('<div class="section-title">🤖 NaqAI</div>', unsafe_allow_html=True)
    if not GEMINI_API_KEY:
        st.warning("أضف GEMINI_API_KEY")
        st.info("🎁 https://aistudio.google.com/app/apikey")
        return
    ...
    examples = ["أفضل زركونيا؟", "ابتسامة لثوية؟", "تبييض نصائح", "فينير أم تاج؟"]
    cols = st.columns(4)
    ...
    with st.form("naq", clear_on_submit=True):
        ...
        if st.form_submit_button("📨") and q:
...
def page_smart_diagnosis():
    ...
    img = image_upload_section("sd", "📸 صورة")
    if img is None:
        st.info("👆 ارفع صورة"); return
    st.image(img, use_container_width=True)
    if st.button("🤖 تشخيص AI", type="primary", use_container_width=True, key="btn_sd"):
        with st.spinner("🤖 AI يحلل الصورة..."):
            ans = ai_analyze_image(img, "قدم تشخيصاً أولياً شاملاً لهذه الحالة: الأسنان، اللثة، الابتسامة، التوصيات")
        st.markdown(f'<div class="ai-msg">🤖 {ans}</div>', unsafe_allow_html=True)
...
def page_dentbook():
    ...
    tab1, tab2 = st.tabs(["📝 نشر", "📰 الأخبار"])
    with tab1:
        with st.form("db", clear_on_submit=True):
            content = st.text_area("ماذا تشارك؟", height=100)
            if st.form_submit_button("🚀 نشر"):
                if content.strip():
                    st.session_state.dentbook_posts.insert(0, {
                        "author": user["name"], "content": content,
                        "time": datetime.now().strftime("%Y-%m-%d %H:%M")
                    })
                    st.rerun()
    with tab2:
        for p in st.session_state.dentbook_posts:
            st.markdown(f'<div class="post-card"><strong style="color:#00d4ff;">{p["author"]}</strong> — {p["time"]}<p>{p["content"]}</p></div>', unsafe_allow_html=True)
...
def page_profile():
    ...
    with st.form("pf"):
        ...
        if st.form_submit_button("💾 حفظ"):
            st.session_state.current_user.update({"name": n, "specialty": s})
...
PAGES = {
    ...
    "ai_simulator": page_ai_simulator, "before_after_simulation": page_before_after_simulation,
    "photorealism": page_photorealism, "dsd_studio": page_dsd_studio,
    "manual_design": page_manual_design, "cad_cam": page_cad_cam,
...
def main():
    ...
    <div style="text-align:center;color:#64748b;font-size:0.8rem;padding:20px;">
        ...
        45+ قسم | AI في كل مكان | تصميم يدوي + تلقائي<br>© 2026
    </div>
    """, unsafe_allow_html=True)
