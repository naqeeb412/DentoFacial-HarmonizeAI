...
# =============================================================
# PAGE: GALLERY (عرض جميع الصور المُنتجة)
# =============================================================
def page_gallery():
    st.markdown('<h2>📸 معرض الصور <span style="color:#e67e22;">المُنتجة</span></h2>', unsafe_allow_html=True)
    st.caption("جميع الصور التي تم إنشاؤها بواسطة النظام")
    ...
    if st.session_state.generated_images:
        ...
        for i, img in enumerate(st.session_state.generated_images):
            with cols[i % 4]:
                st.image(f"data:image/png;base64,{img['data']}", caption=img.get('name', 'صورة'), use_container_width=True)
                st.caption(f"نوع: {img.get('type', 'عام')}")
                st.caption(f"التاريخ: {img.get('created_at', '')[:10]}")
                if st.button("🗑️ حذف", key=f"del_gallery_{i}"):
                    st.session_state.generated_images.pop(i)
                    st.rerun()
    else:
        st.info("لا توجد صور مُنتجة بعد")
