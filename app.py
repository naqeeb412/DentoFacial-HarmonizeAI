...
st.set_page_config(page_title="DENTAL AI OS", page_icon="🦷", layout="wide", initial_sidebar_state="expanded")
...
def ask_gemini(question, context="طب أسنان تجميلي"):
    ...
    try:
        ...
        payload = {"contents": [{"parts": [{"text": f"أنت مساعد طبي متخصص في طب الأسنان التجميلي.\nالسياق: {context}\nالسؤال: {question}\nأجب بالعربية بشكل مختصر ومنظم."}]}], "generationConfig": {"temperature": 0.7, "maxOutputTokens": 1500}}
...
def ai_analyze_image(image, question):
    ...
    try:
        ...
        url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={GEMINI_API_KEY}"
        payload = {"contents": [{"parts": [{"text": f"أنت خبير في طب الأسنان التجميلي.\nالسؤال: {question}\nقدم تحليلاً دقيقاً بالعربية."}, {"inline_data": {"mime_type": "image/jpeg", "data": img_b64}}]}]}
...
def analyze_face_468(image):
    ...
    try:
        ...
        face_width = calc_dist(get_landmark_xy(landmarks, LEFT_CHEEK, w, h), get_landmark_xy(landmarks, RIGHT_CHEEK, w, h))
        ...
        analysis = {"num_landmarks": len(landmarks.landmark), "golden_ratio": phi_ratio, "golden_score": golden_score, "symmetry_score": symmetry_score, "thirds_score": thirds_score, "smile_score": smile_score, "eye_symmetry": eye_sym, "face_shape": face_shape, "overall_score": overall, "grade": grade}
...
def analyze_occlusion(landmarks, w, h):
    try:
        ...
        if overbite < 1 or overbite > 3:
            score -= 15
        return {"angle_class": angle_class, "overjet": overjet, "overbite": overbite, "lips_height": lips_h, "lips_width": lips_w, "occlusion_score": max(0, score), "recommendation": "تقويم مطلوب" if score < 70 else "مراقبة دورية" if score < 90 else "إطباق مثالي"}
    except Exception as e:
        return {"error": str(e)}
...
def analyze_facial_aesthetic(landmarks, w, h):
    try:
        ...
        if abs(mentolabial - 115) > 15:
            score -= 10
        return {"nasolabial_angle": nose_angle, "mentolabial_angle": mentolabial, "facial_thirds": (third1, third2, third3), "aesthetic_score": max(0, score), "grade": "A+ ممتاز" if score > 90 else "A جيد" if score > 80 else "B مقبول"}
    except Exception as e:
        return {"error": str(e)}
...
def cephalometric_analysis(image):
    ...
    h, w = img_np.shape
    analysis = {"SNA": 82.5 + random.uniform(-2, 2), "SNB": 80.0 + random.uniform(-2, 2), "ANB": 2.5 + random.uniform(-1, 1), "SN-MP": 32.0 + random.uniform(-3, 3), "FMA": 25.0 + random.uniform(-3, 3), "IMPA": 90.0 + random.uniform(-3, 3)}
...
def signup_user(name, email, password, specialty=""):
    ...
    st.session_state.users_db[email] = {"name": name, "email": email, "password": hash_pass(password) if password else "", "role": "doctor", "specialty": specialty, "created_at": datetime.now().isoformat()}
...
def sidebar_nav():
    ...
    with st.sidebar:
        ...
        """, unsafe_allow_html=True)
        menu_groups = {
            "🏠 الأساسية": [("🏠 الرئيسية", "home"), ("📊 لوحة التحكم", "dashboard"), ("🏷️ رفع الشعار", "upload_logo")],
            "🧠 التحليل والتشخيص": [("🧠 تحليل الوجه 468", "face_analysis"), ("✨ النسبة الذهبية", "golden_ratio"), ("😊 تحليل الابتسامة", "smile_analysis"), ("🦷 تحليل الإطباق", "occlusion_analysis"), ("💎 تحليل الوجه التجميلي", "facial_aesthetic"), ("🔬 الماسح العلمي", "scientific_scanner"), ("🩻 تحليل الأشعة", "cephalometric")],
            "🎨 التصميم (AI + يدوي)": [("🎨 محاكاة AI", "ai_simulator"), ("🎬 محاكاة قبل/بعد", "before_after_simulation"), ("💎 Photorealism", "photorealism"), ("🧬 استوديو DSD", "dsd_studio"), ("✏️ التصميم اليدوي", "manual_design"), ("🎨 CAD/CAM & 3D", "cad_cam")],
            "📁 إدارة المرضى": [("📁 بيانات المريض", "patient_data"), ("👥 قائمة المرضى", "patients_list"), ("📸 التصوير", "photography"), ("🩻 الأشعة (الأنواع)", "xray_types"), ("📅 المواعيد", "appointments"), ("💰 الحساب", "accounting")],
            "🧪 المواد والعلاج": [("🧪 المواد العلاجية", "materials_guide"), ("📋 خطة العلاج", "treatment_plan"), ("🔄 خط الإنتاج", "pipeline"), ("📊 جداول المقارنات", "comparisons")],
            "👥 الفريق": [("👥 متعدد التخصصات", "multidisciplinary"), ("🗣️ منتدى النقاشات", "discussion_forum"), ("💬 المراسلات", "messages"), ("🧪 المختبر", "lab_chat")],
            "🌍 المنصة والأنظمة": [("🌍 المنصة العالمية", "global_platform"), ("🔌 الأنظمة المستخدمة", "systems_used"), ("📊 التحليلات", "analytics")],
            "📢 الإدارة": [("📢 الإعلانات", "ads_management"), ("👑 الاشتراكات", "subscriptions"), ("📄 التقارير", "reports"), ("⚙️ الإعدادات", "settings")],
            "🤖 AI والتواصل": [("🤖 NaqAI", "naqai"), ("🩺 التشخيص AI", "smart_diagnosis"), ("📱 Dentbook", "dentbook"), ("👤 الملف", "profile"), ("👥 الأعضاء", "members")],
        }
...
def page_face_analysis():
    ...
    if st.session_state.get("last_analysis_image"):
        ...
        if data:
            ...
            chart = pd.DataFrame({"المعيار": ["ذهبية", "تناسق", "أثلاث", "ابتسامة", "عيون"], "النسبة": [data.get('golden_score', 0), data.get('symmetry_score', 0), data.get('thirds_score', 0), data.get('smile_score', 0), data.get('eye_symmetry', 0)]})
            fig = px.bar(chart, x="المعيار", y="النسبة", template="plotly_dark", color="النسبة", color_continuous_scale=["#ef4444", "#10b981", "#00d4ff"])
            fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
            c1, c2, c3 = st.columns(3)
            with c1:
                st.download_button("⬇️ PNG", img_to_bytes(st.session_state.last_analysis_image), "face_468.png", "image/png", use_container_width=True)
            with c2:
                st.download_button("⬇️ JSON", json.dumps(data, ensure_ascii=False, indent=2).encode(), "analysis.json", "application/json", use_container_width=True)
            with c3:
...
def page_golden_ratio():
    ...
    if st.session_state.get("last_golden_image"):
        ...
        st.download_button("⬇️ تحميل", img_to_bytes(st.session_state.last_golden_image), "golden.png", "image/png", use_container_width=True)
...
def page_before_after_simulation():
    st.markdown('<div class="section-title">🎬 محاكاة قبل / بعد</div>', unsafe_allow_html=True)
    ...
    with tab1:
        c1, c2 = st.columns(2)
        with c1:
            treatment = st.selectbox("العلاج", ["تبييض", "زركونيا", "فينير", "تجميلي شامل"], key="ba_treat")
        with c2:
...
def page_ai_simulator():
    ...
    with c2:
        ...
        if st.session_state.get("processed_img"):
            ...
            st.download_button("⬇️", img_to_bytes(st.session_state.processed_img), "sim.png", "image/png", use_container_width=True)
        else:
            st.info("اضغط تطبيق")
...
def page_photorealism():
    ...
    with c2:
        if st.session_state.get("last_photorealism_image"):
            ...
            st.download_button("⬇️", img_to_bytes(st.session_state.last_photorealism_image), "ph.png", "image/png", use_container_width=True)
        else:
            st.image(img, use_container_width=True)
...
def page_dsd_studio():
    ...
    tab1, tab2 = st.tabs(["🤖 AI Auto", "✏️ Manual"])
    with tab1:
        tooth_color = st.selectbox("🎨 لون", ["A1", "A2", "A3", "B1", "Hollywood"], key="dsd_ai_c")
        if st.button("🧬 تصميم AI", type="primary", use_container_width=True, key="btn_dsd_ai"):
            with st.spinner("🧬..."):
                ...
                colors = {"A1": [240, 235, 220], "A2": [235, 225, 205], "A3": [225, 210, 185], "B1": [230, 225, 210], "Hollywood": [255, 255, 250]}
    ...
    if st.session_state.get("last_dsd_image"):
        ...
        st.download_button("⬇️", img_to_bytes(st.session_state.last_dsd_image), "dsd.png", "image/png", use_container_width=True)
...
def page_manual_design():
    ...
    with tab1:
        ...
        if st.button("🎨 تطبيق", type="primary", use_container_width=True, key="btn_md_filters"):
            with st.spinner("🎨..."):
                ...
                st.rerun()
    with tab2:
        draw_type = st.selectbox("نوع", ["دائرة", "مستطيل", "خط"], key="md_draw_type")
        color = st.color_picker("اللون", "#00d4ff", key="md_color")
        width = st.slider("السماكة", 1, 10, 3, key="md_width")
        if st.button("✏️ إضافة", use_container_width=True, key="btn_md_draw"):
            with st.spinner("✏️..."):
                result = img.copy()
    ...
    with tab3:
        ...
        if st.button("📝 إضافة", use_container_width=True, key="btn_md_text"):
            with st.spinner("📝..."):
                result = img.copy()
                draw = ImageDraw.Draw(result)
                try:
    ...
    if st.session_state.get("processed_img"):
        st.image(st.session_state.processed_img, use_container_width=True)
        st.download_button("⬇️", img_to_bytes(st.session_state.processed_img), "design.png", "image/png", use_container_width=True)
    else:
        st.image(img, use_container_width=True)

def page_cad_cam():
    st.markdown('<div class="section-title">🎨 CAD/CAM & 3D</div>', unsafe_allow_html=True)
    tab1, tab2, tab3 = st.tabs(["🎨 توليد AI", "📦 STL", "⚙️ الأنظمة"])
    with tab1:
        prompt = st.text_area("📝 وصف")
        if st.button("🎨 توليد AI", type="primary", use_container_width=True, key="btn_3d_ai"):
            with st.spinner("🎨..."):
                if GEMINI_API_KEY:
                    ans = ask_gemini(f"اقترح مواصفات 3D لنموذج: {prompt}. قدم: الأبعاد، المادة، خطوات التصميم")
                    st.markdown(f'<div class="ai-msg">🤖 {ans}</div>', unsafe_allow_html=True)
                else:
    ...
    with tab1:
        with st.form("patient_info"):
            ...
            if st.form_submit_button("💾 حفظ", use_container_width=True):
                st.session_state.patient_files.append({"name": name, "age": age, "gender": gender, "phone": phone, "treatment": treatment, "cost": cost, "time": datetime.now().strftime("%Y-%m-%d %H:%M")})
                st.rerun()
...
def page_xray_types():
    ...
    types = {"سيفالومترية": "لتحليل الفكين", "بانورامية": "لرؤية كل الأسنان", "CBCT": "تصوير 3D", "Periapical": "تفاصيل السن", "Occlusal": "سقف/أرضية الفم", "Bitewing": "التسوس"}
...
def page_materials_guide():
    ...
    if GEMINI_API_KEY:
        ...
        if st.button("🤖 اسأل AI", use_container_width=True, key="btn_ask_mat"):
            with st.spinner("🤖..."):
                ans = ask_gemini(f"اشرح مادة {mat_sel}: استخدام، ميزات، بدائل، نصائح")
            st.markdown(f'<div class="ai-msg">🤖 {ans}</div>', unsafe_allow_html=True)
...
def page_treatment_plan():
    ...
    c1, c2 = st.columns(2)
    with c1:
        main = st.text_area("الخطة")
    with c2:
        alt = st.text_area("البديل")
...
def page_pipeline():
    ...
    steps = [("1️⃣ الاستشارة", "done"), ("2️⃣ التحضير", "done"), ("3️⃣ CAD/CAM", "active"), ("4️⃣ التصنيع", "pending"), ("5️⃣ التسليم", "pending")]
    ...
    fig = go.Figure(go.Indicator(mode="gauge+number", value=58, title={'text': "الإنجاز"}, gauge={'bar': {'color': "#00d4ff"}}))
    st.plotly_chart(fig, use_container_width=True)
...
def page_comparisons():
    ...
    with tab1:
        df = pd.DataFrame({"المعيار": ["لون الأسنان", "تناسق", "اللثة", "التناسب", "الثقة"], "قبل": [45, 60, 70, 55, 50], "بعد": [95, 92, 90, 88, 98], "التحسن": ["+111%", "+53%", "+29%", "+60%", "+96%"]})
    ...
    with tab2:
        ...
        comp = pd.DataFrame({"المعيار": ["أسنان", "تكلفة", "مدة", "رضا"], "الحد الأدنى": [df['عدد_الأسنان'].min(), df['التكلفة_ريال'].min(), df['المدة_شهر'].min(), df['رضا_المريض_%'].min()], "الأعلى": [df['عدد_الأسنان'].max(), df['التكلفة_ريال'].max(), df['المدة_شهر'].max(), df['رضا_المريض_%'].max()], "المتوسط": [df['عدد_الأسنان'].mean(), df['التكلفة_ريال'].mean(), df['المدة_شهر'].mean(), df['رضا_المريض_%'].mean()]})
...
def page_discussion_forum():
    ...
    with st.form("forum_form", clear_on_submit=True):
        ...
        if st.form_submit_button("🚀 نشر", use_container_width=True):
            if title and body:
                st.session_state.discussion_forum.insert(0, {"title": title, "body": body, "asked_by": st.session_state.current_user["name"], "time": datetime.now().strftime("%Y-%m-%d %H:%M")})
                st.rerun()
    for i, q in enumerate(st.session_state.discussion_forum):
        ...
        if GEMINI_API_KEY and st.button("🤖 إجابة AI", key=f"ai_ans_{i}"):
            with st.spinner("🤖..."):
                ans = ask_gemini(f"أجب عن: {q['title']} - {q['body']}")
            st.markdown(f'<div class="ai-msg">🤖 {ans}</div>', unsafe_allow_html=True)
...
def page_global_platform():
    ...
    stages = [("1️⃣ التحضير", "done"), ("2️⃣ التشخيص", "done"), ("3️⃣ CAD", "active"), ("4️⃣ CAM", "pending"), ("5️⃣ التسليم", "pending")]
    for name, status in stages:
        icon = "✅" if status == "done" else "🔄" if status == "active" else "⏳"
        st.markdown(f'<div class="timeline-step timeline-{status}"><div>{icon}</div><strong>{name}</strong></div>', unsafe_allow_html=True)
    cases = pd.DataFrame({"الدولة": ["اليمن", "السعودية", "الإمارات", "مصر"], "الحالات": [24, 45, 32, 18], "النجاح": ["94%", "96%", "92%", "88%"]})
    st.dataframe(cases, use_container_width=True, hide_index=True)

def page_systems_used():
    st.markdown('<div class="section-title">🔌 الأنظمة المستخدمة</div>', unsafe_allow_html=True)
    systems = [("Meshy AI", "توليد 3D", "https://www.meshy.ai"), ("Blender", "3D", "https://www.blender.org"), ("Exocad", "CAD", "https://exocad.com"), ("3Shape", "مسح", "https://www.3shape.com"), ("Gemini AI", "AI", "https://ai.google.dev")]
    for name, desc, url in systems:
        st.markdown(f'<div class="card"><strong style="color:#00d4ff;">{name}</strong> — {desc} <a href="{url}" target="_blank">🔗</a></div>', unsafe_allow_html=True)
...
def page_analytics():
    ...
    fig = px.bar(edited.groupby('نوع_العلاج')['التكلفة_ريال'].sum().reset_index(), x='نوع_العلاج', y='التكلفة_ريال', template='plotly_dark')
    ...
    with tab2:
        with st.form("ext_ad"):
            ...
            if st.form_submit_button("🌐 نشر"):
                ...
                st.success(f"✅ على {len(platforms)} منصة")
...
def page_subscriptions():
    ...
    for i, (name, price) in enumerate(plans):
        with cols[i]:
            ...
            if st.button("اشترك", key=f"sub_{i}"):
                st.success("🎉")
...
def page_reports():
    ...
    imgs = {}
    if st.session_state.get("last_analysis_image"):
        imgs["تحليل الوجه"] = st.session_state.last_analysis_image
    if st.session_state.get("last_golden_image"):
        imgs["النسبة الذهبية"] = st.session_state.last_golden_image
    if st.session_state.get("last_cephalometric_image"):
        imgs["الأشعة"] = st.session_state.last_cephalometric_image
    if st.button("📄 توليد", type="primary", key="btn_rep"):
        if imgs:
            html = f'<html dir="rtl"><head><meta charset="UTF-8"><title>تقرير</title></head><body><h1>تقرير {n}</h1><p>{datetime.now().strftime("%Y-%m-%d")}</p>'
...
def page_dentbook():
    ...
    with tab1:
        with st.form("db", clear_on_submit=True):
            ...
            if st.form_submit_button("🚀 نشر"):
                if content.strip():
                    st.session_state.dentbook_posts.insert(0, {"author": user["name"], "content": content, "time": datetime.now().strftime("%Y-%m-%d %H:%M")})
                    st.rerun()
    with tab2:
...
def main():
    ...
    try:
        ...
        if page_func is None:
            st.warning(f"الصفحة غير موجودة")
            page_func = page_home
        page_func()
    except Exception as e:
    ...
    <div style="text-align:center;color:#64748b;font-size:0.8rem;padding:20px;">
        <strong style="color:#00d4ff;">DENTAL AI OS v7.0 Pro</strong><br>© 2026
    </div>
    """, unsafe_allow_html=True)
