...
# =============================================================
st.set_page_config(
    page_title="HarmonizeAI™ | Dental Platform",
    page_icon="🦷",
    layout="wide",
...
def signup(name, email, password, confirm_password, role, specialty="", phone=""):
...
def page_auth():
    ...
    <div class="main-header">
        ...
        <p class="subtitle">منصة طب الأسنان الرقمي المتكاملة</p>
    </div>
    """, unsafe_allow_html=True)
        ...
        with tab2:
            with st.form("signup_form"):
                ...
                role = st.selectbox("👥 نوع الحساب", ["doctor", "patient"], 
                                   format_func=lambda x: "طبيب" if x == "doctor" else "مريض")
                
                if role == "doctor":
                    specialty = st.text_input("🎯 التخصص (اختياري)", placeholder="مثال: تقويم أسنان")
                else:
                    specialty = ""
                ...
                if submit:
                    success, message = signup(name, email, password, confirm_password, role, specialty, phone)
                    if success:
                        st.success(f"✅ {message}")
...
def page_home():
    ...
    if st.session_state.current_user:
        ...
        if user.get("role") == "owner":
            role_badge = '<span class="owner-badge">👑 مالك النظام</span>'
        elif user.get("role") == "doctor":
            role_badge = '<span class="owner-badge" style="background:#10b981;color:white;">👨‍⚕️ طبيب</span>'
        else:
            role_badge = '<span class="owner-badge" style="background:#3b82f6;color:white;">👤 مريض</span>'
        
        st.markdown(f"""
        <div style="text-align:center;margin:20px 0;">
            <h3>مرحباً، {user['name']} {role_badge}</h3>
            <p style="color:#94a3b8;">{user.get('specialty', '')}</p>
            <p style="color:#64748b;font-size:0.9rem;">عضو منذ {user.get('created_at', '')}</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class="metric-card">
            <h2 style="color:#e67e22;margin:0;">👥</h2>
            <h3 style="margin:10px 0;">{}</h3>
            <p style="color:#94a3b8;margin:0;">المرضى</p>
        </div>
    ...
    with col2:
        ...
        <div class="metric-card">
            ...
            <p style="color:#94a3b8;margin:0;">الأسنان</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        <div class="metric-card">
            <h2 style="color:#e67e22;margin:0;">📊</h2>
            <h3 style="margin:10px 0;">478</h3>
            <p style="color:#94a3b8;margin:0;">نقطة قياس</p>
        </div>
        """, unsafe_allow_html=True)
    
    with col4:
        users_count = len(st.session_state.users_db)
        st.markdown("""
        <div class="metric-card">
            <h2 style="color:#e67e22;margin:0;">👤</h2>
            <h3 style="margin:10px 0;">{}</h3>
            <p style="color:#94a3b8;margin:0;">مستخدم</p>
        </div>
        """.format(users_count), unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class="feature-card">
            ...
            <h3>محاكي الابتسامة</h3>
            <p style="color:#94a3b8;">تحليل وتحسين الابتسامة باستخدام الذكاء الاصطناعي</p>
        </div>
        """, unsafe_allow_html=True)
        
        if st.button("ابدأ المحاكاة", key="btn_smile", use_container_width=True):
            st.session_state.current_page = "smile"
            st.rerun()
    ...
    with col2:
        ...
        <div class="feature-card">
            <h2 style="font-size:3rem;margin:0;">🦷</h2>
            <h3>المخطط السني</h3>
            <p style="color:#94a3b8;">مخطط تفاعلي لتتبع حالة كل سن</p>
        </div>
        """, unsafe_allow_html=True)
        
        if st.button("افتح المخطط", key="btn_dental", use_container_width=True):
            st.session_state.current_page = "dental"
            st.rerun()
    ...
    with col3:
        ...
        <div class="feature-card">
            <h2 style="font-size:3rem;margin:0;">👥</h2>
            <h3>إدارة المرضى</h3>
            <p style="color:#94a3b8;">إدارة سجلات المرضى والمواعيد</p>
        </div>
        """, unsafe_allow_html=True)
        
        if st.button("إدارة المرضى", key="btn_patients", use_container_width=True):
...
# =============================================================
def page_smile():
    st.markdown("## 😊 محاكي الابتسامة")
    st.caption("ارفع صورة لتجربة تحسين الابتسامة")
    ...
    if uploaded_file:
        ...
        intensity = st.slider("🎚️ شدة التحسين", 0.0, 1.0, 0.5, 0.1)
        ...
        if st.button("✨ تحسين الابتسامة", type="primary", use_container_width=True):
            with st.spinner("جاري المعالجة..."):
                ...
                with col2:
                    st.image(img, caption="بعد التحسين", use_container_width=True)
    ...
    if st.session_state.smile_results:
        st.markdown("---")
        st.markdown("### 📋 سجل التحسينات")
        
        for i, result in enumerate(st.session_state.smile_results):
            with st.expander(f"نتيجة {i+1} - {result['timestamp']} - بواسطة {result.get('user', '')}"):
                col1, col2 = st.columns(2)
                with col1:
                    st.image(result["original"], caption="قبل", use_container_width=True)
                with col2:
                    st.image(result["enhanced"], caption="بعد", use_container_width=True)

# =============================================================
# PAGE: DENTAL CHART
# =============================================================
def page_dental():
    st.markdown("## 🦷 المخطط السني التفاعلي")
    st.caption("تتبع حالة الأسنان")
    
    # Display dental chart
    st.markdown("### الفك العلوي")
    
    # Create columns for teeth
    cols = st.columns(16)
    
    for i in range(16):
        with cols[i]:
            tooth_status = st.session_state.dental_chart[i]
            
            if tooth_status == "normal":
                emoji = "🦷"
            elif tooth_status == "missing":
                emoji = "❌"
            elif tooth_status == "carious":
                emoji = "🔴"
            elif tooth_status == "treated":
                emoji = "✅"
            elif tooth_status == "crown":
                emoji = "👑"
            else:
                emoji = "🦷"
            
            st.markdown(f"""
            <div style="text-align:center;padding:10px;background:#1e293b;border-radius:8px;margin:2px;">
                <div style="font-size:24px;">{emoji}</div>
                <small style="color:#94a3b8;">{i+1}</small>
            </div>
            """, unsafe_allow_html=True)
    ...
    with col1:
        tooth_number = st.number_input("🔢 رقم السن", 1, 32, 1)
    
    with col2:
        status_label = st.selectbox("📋 الحالة", ["سليم", "مفقود", "نخر", "معالج", "تاج"])
    ...
    with col3:
        ...
        if st.button("تحديث الحالة", use_container_width=True):
            ...
            st.session_state.dental_chart[tooth_number - 1] = status_map[status_label]
...
def page_patients():
    ...
    with st.expander("➕ إضافة مريض جديد", expanded=False):
        with st.form("add_patient"):
            ...
            with col1:
                ...
                age = st.number_input("العمر", 0, 100, 30)
            ...
            if submit:
                if name:
                    ...
                    st.success(f"✅ تم إضافة المريض {name}")
                    st.rerun()
                else:
    ...
    if st.session_state.patients:
        ...
        # Search
        search = st.text_input("🔍 بحث", placeholder="ابحث بالاسم أو الهاتف...")
        
        filtered = st.session_state.patients
        if search:
            filtered = [p for p in st.session_state.patients 
                       if search.lower() in p["name"].lower() 
                       or search in p.get("phone", "")]
        ...
        if filtered:
            ...
            with col2:
                males = len([p for p in st.session_state.patients if p.get("gender") == "ذكر"])
                st.metric("الذكور", males)
            with col3:
                females = len([p for p in st.session_state.patients if p.get("gender") == "أنثى"])
...
# =============================================================
# PAGE: USERS MANAGEMENT (للمالك فقط)
# =============================================================
def page_users():
    if st.session_state.current_user.get("role") != "owner":
        st.error("❌ هذه الصفحة مخصصة لمالك النظام فقط")
        return
    
    st.markdown("## 👥 إدارة المستخدمين")
    st.caption("صفحة مخصصة لمالك النظام")
    ...
    if st.session_state.users_db:
        ...
        for email, user_data in st.session_state.users_db.items():
            users_list.append({
                ...
                "التخصص": user_data.get("specialty", ""),
                "الهاتف": user_data.get("phone", ""),
                "تاريخ التسجيل": user_data.get("created_at", "")
            })
        ...
        doctors = len([u for u in st.session_state.users_db.values() if u["role"] == "doctor"])
        patients = len([u for u in st.session_state.users_db.values() if u["role"] == "patient"])
        
        with col1:
            st.metric("إجمالي المستخدمين", total_users)
        with col2:
            st.metric("الأطباء", doctors)
        with col3:
            st.metric("المرضى", patients)
...
def sidebar():
    with st.sidebar:
        ...
        if st.session_state.authenticated and st.session_state.current_user:
            ...
            role_icon = "👑" if user.get("role") == "owner" else ("👨‍⚕️" if user.get("role") == "doctor" else "👤")
            
            st.markdown(f"### {role_icon} {user['name']}")
            st.caption(user.get('specialty', ''))
            ...
            if st.button("😊 محاكي الابتسامة", use_container_width=True):
                st.session_state.current_page = "smile"
                st.rerun()
            
            if st.button("🦷 المخطط السني", use_container_width=True):
                st.session_state.current_page = "dental"
                st.rerun()
            
            if st.button("👥 المرضى", =True):
                st.session_state.current_page = "patients"
                st.rerun()
            
            # زر إدارة المستخدمين للمالك فقط
            if user.get("role") == "owner":
                st.markdown("---")
        ...
        st.caption("HarmonizeAI™ v1.0")
        st.caption("© 2024 جميع الحقوق محفوظة")
        st.caption("Developed by: علي النقيب")
