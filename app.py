...
def show_home():
    ...
    st.write("مرحباً بك في النظام الذكي المتكامل للتشخيص وتصميم الابتسامة والتجميل السني.")
    st.info("قم بتسجيل الدخول للاستفادة من كافة أدوات المعالجة، إدارة المرضى، والتواصل مع الزملاء.")
...
def show_dashboard():
    ...
    col1.metric("المرضى المسجلون", len(st.session_state.patients))
    col2.metric("منشورات Dentbook", len(st.session_state.dentbook_posts))
    col3.metric("حالة النظام", "نشط 🟢")

def show_patients():
    st.title("👨‍⚕️ إدارة المرضى")
    df = pd.DataFrame(st.session_state.patients)
    if not df.empty:
        st.dataframe(df[['patient_id', 'name', 'phone', 'age', 'gender', 'notes']], use_container_width=True)
    else:
        st.info("لا يوجد مرضى مسجلون حتى الآن.")
...
def show_new_patient():
    ...
    with st.form("p_form"):
        ...
        notes = st.text_area("ملاحظات التشخيص")
...
def show_dentbook():
    st.title("🌐 شبكة Dentbook الاجتماعية")
    st.write("مشاركة الحالات العلمية والنقاشات الطبية.")
    with st.form("post_form"):
        post_text = st.text_area("ماذا يدور في ذهنك طبياً؟")
        if st.form_submit_button("نشر"):
            if post_text:
                user = st.session_state.user
                add_dentbook_post(user['uid'], user['name'], "", user['email'], post_text)
                st.session_state.dentbook_posts = get_dentbook_posts()
    ...
    for p in posts:
        st.markdown(f"**{p['author_name']}** - *{p['timestamp']}*")
        st.write(p['text'])
        st.divider()

def show_group_chat():
    st.title("💬 الدردشة الجماعية الطبية")
...
def show_lab():
    st.title("🧪 معمل الأسنان الافتراضي")
    st.write("إدارة الطلبات والملفات الخاصة بالمختبر.")

def show_forum():
    st.title("❓ الأسئلة والاستشارات")
    st.write("طرح الحالات المعقدة وطلب استشارة الخبراء.")

def show_materials():
    st.title("📦 مواد المعالجة")
    st.write("إدارة المواد المخزنة والمستخدمة في العيادة.")

def show_ads():
    st.title("📢 الإعلانات والعروض")
    st.write("لوحة الإعلانات الطبية والشركات الداعمة.")
