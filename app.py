...
def authenticate(email, password):
    # ✅ حساب افتراضي خاص بك (يعمل دائماً)
    ...
    # محاولة المصادقة من قاعدة البيانات
...
def show_dashboard():
    ...
    # زر تحميل الشعار
    st.button("⬇️ تحميل الشعار", on_click=lambda: st.download_button(
        label="تحميل الشعار",
        data="شعار HarmonizeAI",
        file_name="HarmonizeAI_Logo.txt",
        mime="text/plain"
    ))
    ...
    if st.session_state.patients:
        ...
        st.info("لا يوجد مرضى. أضف مريضاً جديداً من القائمة الجانبية.")
    
    # نبذة عن النظام
    with st.expander("ℹ️ نبذة عن النظام", expanded=True):
        st.write("""
        **Dentofacial HarmonizeAI™** هي منصة متكاملة لتشخيص وعلاج الوجه والأسنان بالذكاء الاصطناعي، 
        تهدف إلى تقديم حلول رقمية متطورة في مجال طب الأسنان التجميلي وعلاج الوجه. 
        توفر المنصة تحليل الوجه بدقة 478 علامة تشريحية، تحليل الأشعة، تصميم الابتسامة، 
        ومحاكاة نتائج العلاج قبل البدء به، مع إمكانية التواصل بين الأطباء والمرضى عبر شبكة اجتماعية طبية متكاملة.
        """)
    ...
    if st.session_state.patients:
        ...
        for p in st.session_state.patients:
            with st.expander(f"👤 {p['name']}"):
                ...
                if st.button(f"🗑️ حذف {p['name']}", key=f"del_{p['id']}"):
                    delete_patient(p['id'])
                    st.session_state.patients = get_patients()
                    st.rerun()
    else:
...
def show_diagnosis():
    ...
    if st.button("تشخيص AI - Harvard"):
        if symptoms:
            if "ألم" in symptoms.lower():
                diagnosis = "ألم في المنطقة. التهاب لثة. سوء إطباق."
                recommendations = "فحص سريري، تنظيف عميق، تقويم أسنان."
            else:
                diagnosis = "لا توجد أعراض واضحة. يوصى بفحص سريري."
                recommendations = "فحص شامل"
            
            st.success(f"✅ التشخيص: {diagnosis}")
            st.info(f"📌 التوصيات: {recommendations}")
        else:
...
if __name__ == "__main__":
    main()    c.execute('''CREATE TABLE IF NOT EXISTS group_messages (
        id TEXT PRIMARY KEY,
        sender TEXT,
        sender_email TEXT,
        recipient TEXT DEFAULT 'group',
