...
# Users Database - مع بيانات افتراضية للتسجيل الدخول
...
def auth_page():
    ...
    with col2:
        ...
        st.info("""
        **📧 حسابات تجريبية:**
        - **المالك:** ndcdental2025@outlook.com / ndc2025
        ...
        for f in friends:
            ...
            if friend_user:
                ...
                <div class="card" style="padding:12px; display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div><button onclick="alert('💬 تم فتح المحادثة')" style="background:#0a8491; color:#fff; border:none; padding:2px 12px; border-radius:20px; cursor:pointer;">💬</button></div>
                </div>
                """, unsafe_allow_html=True)
    else:
...
def page_profile():
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg, #075e68, #0a8491); border-radius:12px 12px 0 0; height:160px; position:relative;">
        <button onclick="alert('تغيير الغلاف')" style="position:absolute; bottom:8px; left:8px; background:rgba(0,0,0,0.5); color:#fff; border:none; padding:4px 14px; border-radius:30px; font-size:0.7rem; cursor:pointer;">تغيير الغلاف</button>
    </div>
    <div style="display:flex; align-items:flex-end; gap:16px; padding:0 16px 16px; margin-top:-40px; flex-wrap:wrap;">
        <div style="width:80px; height:80px; border-radius:50%; border:4px solid #1e293b; background:#0a8491; display:flex; align-items:center; justify-content:center; font-size:32px; color:#fff;">
            {user['name'][0]}
        </div>
        <div>
            <h3>{user['name']}</h3>
            ...
            for model in st.session_state.converted_models[-5:]:
                ...
                <div class="card" style="padding:12px;">
                    <div style="display:flex; justify-content:space-between;">
                        ...
                        <div><span style="color:#10b981;">✅ {model['status']}</span></div>
                    </div>
...
def page_image_to_3d():
    ...
    if uploaded:
        ...
        st.selectbox("صيغة الخرج", ["STL", "OBJ", "PLY", "GLB"])
        if st.button("🔄 توليد النموذج 3D", type="primary", use_container_width=True):
            with st.spinner("⏳ جاري توليد النموذج ثلاثي الأبعاد..."):
                ...
                st.markdown("""
                <div style="width:100%; height:300px; background:#0f172a; border-radius:16px; border:1px solid #334155; display:flex; align-items:center; justify-content:center;">
                ...
                """, unsafe_allow_html=True)
                st.download_button(label="⬇️ تحميل النموذج", data=b"3D Model Data", file_name="3d_model.stl", mime="application/octet-stream")
        ...
        for i, model in enumerate(st.session_state.converted_models):
            with cols[i % 3]:
                ...
                <div class="card" style="padding:16px; text-align:center;">
                    ...
                    <div style="margin-top:8px;"><span style="background:#10b981; color:#fff; padding:2px 12px; border-radius:20px; font-size:0.6rem;">✅ {model['status']}</span></div>
                </div>
                """, unsafe_allow_html=True)
    else:
        st.info("لا توجد نماذج محولة بعد")
